/**
 * Chargeback Analytics Dashboard - Mobile-First Redesign
...
export default function ChargebackAnalyticsPage() {
  ...
  useEffect(() => {
    ...
    setDateFrom(from);
    setDateTo(to);
    fetchAnalytics(from, to, selectedClient, selectedRange);
  ...
  useEffect(() => {
    ...
    setDateTo(to);
    fetchAnalytics(from, to, selectedClient, selectedRange);
  }, [selectedRange, deriveRange, fetchAnalytics, selectedClient]);
  ...
  const handleRefresh = () => {
    fetchAnalytics(dateFrom, dateTo, selectedClient, selectedRange);
  };
  ...
  const handleExport = async () => {
    try {
      ...
      await AnalyticsApiService.downloadCsv('/analytics/chargeback_sla/', body, `chargeback_sla_${from}_${to}.csv`)
      toast.success('Chargeback SLA CSV downloaded')
    } catch (e) {
      toast.error('Failed to export Chargeback SLA')
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Card className="bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-xl shadow-lg overflow-hidden">
            <CardHeader className="p-4 md:p-6 pb-3">
              <CardTitle className="text-sm md:text-base font-extrabold text-gray-900" style={{ letterSpacing: '-0.02em' }}>Monthly Trend Details</CardTitle>
            </CardHeader>
            <CardContent className="p-4 md:p-6 pt-0">
              {/* Mobile Card View */}
                  ...
                  <div key={index} className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                        ...
                        <span className="font-semibold text-gray-900 ml-2">{month.totalChargebacks}</span>
                      </div>
                      <div>
                        <span className="text-gray-600">Won:</span>
                        <span className="font-semibold text-green-600 ml-2">{month.wonChargebacks}</span>
                      </div>
                      <div>
                        <span className="text-gray-600">Lost:</span>
                        <span className="font-semibold text-red-600 ml-2">{month.lostChargebacks}</span>
                      </div>
                      <div>
                        <span className="text-gray-600">Amount:</span>
                        <span className="font-semibold text-gray-900 ml-2">{formatCurrency(month.amount)}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-extrabold text-gray-700 uppercase" style={{ letterSpacing: '-0.02em' }}>Month</th>
                      <th className="px-4 py-3 text-center text-xs font-extrabold text-gray-700 uppercase" style={{ letterSpacing: '-0.02em' }}>Total</th>
                      <th className="px-4 py-3 text-center text-xs font-extrabold text-gray-700 uppercase" style={{ letterSpacing: '-0.02em' }}>Won</th>
                      <th className="px-4 py-3 text-center text-xs font-extrabold text-gray-700 uppercase" style={{ letterSpacing: '-0.02em' }}>Lost</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {analytics.monthlyTrend.map((month, index) => (
                      <tr key={index} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-sm text-center font-semibold text-gray-900">{month.totalChargebacks}</td>
                        <td className="px-4 py-3 text-sm text-center font-semibold text-green-600">{month.wonChargebacks}</td>
                        <td className="px-4 py-3 text-sm text-center font-semibold text-red-600">{month.lostChargebacks}</td>
                        <td className="px-4 py-3 text-sm text-right font-semibold text-gray-900">{formatCurrency(month.amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <Card className="bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-xl shadow-lg overflow-hidden">
            <CardHeader className="p-4 md:p-6 pb-3">
              <CardTitle className="text-sm md:text-base font-extrabold text-gray-900" style={{ letterSpacing: '-0.02em' }}>Performance Metrics</CardTitle>
            </CardHeader>
            <CardContent className="p-4 md:p-6 pt-0">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                {/* Status Distribution */}
                <div className="bg-white rounded-xl border border-gray-200 p-4 md:p-6">
                  <h3 className="font-extrabold text-gray-900 mb-4 text-sm md:text-base" style={{ letterSpacing: '-0.02em' }}>Status Distribution</h3>
                  <div className="space-y-3">
                    {Object.entries(analytics.statusDistribution).map(([status, count]) => (
                      <div key={status} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                        <span className="text-xs md:text-sm capitalize text-gray-700">{status.replace('_', ' ')}</span>
                        <span className="font-semibold text-sm md:text-base text-gray-900">{count}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Response Time */}
                <div className="bg-white rounded-xl border border-gray-200 p-4 md:p-6">
                  <h3 className="font-extrabold text-gray-900 mb-4 text-sm md:text-base" style={{ letterSpacing: '-0.02em' }}>Average Response Time</h3>
                  <div className="text-center p-6 md:p-8 bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl border border-orange-200">
                    <div className="text-4xl md:text-5xl font-bold mb-2 bg-gradient-to-r from-orange-500 to-orange-600 bg-clip-text text-transparent">
                      {analytics.averageResponseTime.toFixed(1)}
                    </div>
                    <p className="text-xs md:text-sm text-gray-700 font-medium">Hours (Average)</p>
                  </div>
                </div>
              </div>
            </CardContent>
