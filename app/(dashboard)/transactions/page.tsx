                ...
                <tbody className="divide-y divide-gray-200">
                  {rowsToRender.map((txn, index) => (
                    <tr key={txn.txn_id || index} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900 font-mono">{txn.client_txn_id || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.pg_txn_id || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.challan_no || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.pg_pay_mode || '-'}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-900 font-medium">{formatCurrency(txn.payee_amount || 0)}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-700">{formatCurrency(txn.act_amount || 0)}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-700">{formatCurrency(txn.paid_amount || 0)}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-700">{formatCurrency(txn.pg_return_amount || 0)}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.amount_type || '-'}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-700">{txn.p_convcharges || '-'}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-700">{txn.p_ep_charges || '-'}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-700">{txn.p_gst || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.ep_conv_rate || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.ep_conv_rate_type || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.sp_conv_rate || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.sp_conv_rate_type || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.gst_rate || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.gst_rate_type || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{formatDate(txn.trans_date)}</td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${getStatusBadge(txn.status)}`}>
                          {txn.status || '-'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.payee_first_name || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.payee_lst_name || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.payee_mob || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.payee_email || '-'}</td>
                      <td className="px-6 py-4 text-sm text-orange-500 font-medium">{txn.client_code || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.payment_mode || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700 max-w-xs truncate">{txn.payee_address || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.udf1 || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.udf2 || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.udf3 || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.udf4 || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.udf5 || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.udf6 || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.udf9 || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.udf10 || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.udf20 || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.gr_number || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.fee_forward || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700 max-w-xs truncate">{txn.bank_message || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.ifsc_code || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.payer_acount_number || '-'}</td>
                      <td className="px-6 py-4 text-sm text-gray-700">{txn.bank_txn_id || '-'}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-700">{formatCurrency(txn.donation_amount || 0)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
