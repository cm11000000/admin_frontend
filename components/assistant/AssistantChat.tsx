...
export default function AssistantChat() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2 border-b border-gray-200/80 bg-white/90">
        <div className="flex items-center justify-between">
          <div className="text-sm font-extrabold text-gray-700" style={{ letterSpacing: '-0.02em' }}>Ops Assistant</div>
          <div className="flex items-center gap-2">
            <label className="text-xs font-extrabold text-gray-500" style={{ letterSpacing: '-0.02em' }}>Language</label>
            <select
              value={lang}
              onChange={(e) => setLang(e.target.value as any)}
              className="text-xs border border-gray-200 rounded-md px-2 py-1 bg-white text-gray-700 font-medium"
            >
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              ...
              disabled={busy}
              onClick={() => ask(chip.q)}
              className="text-xs font-medium rounded-full border border-gray-200/80 bg-white/90 hover:bg-gray-50 px-3 py-1.5 shadow-sm disabled:opacity-50 transition-all"
            >
              {chip.label}
            </button>
          ))}
      ...
      <div className="flex-1 px-3 py-2 overflow-y-auto">
        {messages.length === 0 && (
          <div className="text-sm font-light text-gray-500" style={{ letterSpacing: '-0.01em' }}>
            Examples: "Show refunds for QCCLI last 7 days", "What's the status of QCCLI-SBI-TODAY?", "Download settlement report last 7 days".
          </div>
        )}
        {messages.map((m, i) => (
                ...
                <div className="mt-2 space-y-2">
                  {m.raw.download?.url && (
                    <button
                      ...
                      className="text-sm font-semibold rounded-lg bg-gradient-to-r from-[#FF9933] to-[#FF6600] text-white px-3 py-1.5 shadow hover:opacity-95 transition-all"
                    >
                      Download File →
                    </button>
                  )}
      ...
      {/* Input */}
      <form onSubmit={onSubmit} className="border-t border-gray-200 px-3 py-3 bg-white/90">
        <div className="flex items-center gap-2">
          <input
            value={input}
          ...
          <button
            type="submit"
            disabled={busy}
            className="min-h-[40px] rounded-xl bg-gradient-to-r from-[#5CBBF6] to-[#5CBBF6]/80 text-white text-sm font-semibold px-4 py-2 shadow hover:opacity-95 disabled:opacity-50 transition-all"
          >
            {busy ? 'Sending...' : 'Send →'}
          </button>
        </div>
